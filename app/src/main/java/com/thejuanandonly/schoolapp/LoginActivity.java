    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == RESULT_LOAD_IMAGE && resultCode == RESULT_OK && null != data) {
           ...
           try {
            ...
            int w = bitmap.getWidth(), h = bitmap.getHeight(); //Crashes here
            int radius = w > h ? h:w;

            Bitmap roundBitmap = ImageToCircleLOGIN.getCroppedBitmap(bitmap, radius);

            */
            avatar_circle.setImageURI(avatarURI);
